...
class ReservationControllerTest extends BaseContainerIntegrationTest {
    ...
    @Test
    @WithMockUser(value = "user1@example.com")
    @DisplayName("상태별 필터링된 보낸 예약 목록 조회 - RETURN_COMPLETED 상태")
    void getSentReservationsWithStatusFilterTest() throws Exception {
        // reservation ID 1의 상태가 RETURN_COMPLETED
        mockMvc.perform(get("/api/v1/reservations/sent")
                        .param("status", "RETURN_COMPLETED")
                        .param("page", "0")
                        .param("size", "5"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value(200))
    ...
    @Test
    @WithMockUser(value = "user1@example.com")
    @DisplayName("키워드로 보낸 예약 검색 테스트")
    void getSentReservationsWithKeywordTest() throws Exception {
        mockMvc.perform(get("/api/v1/reservations/sent")
                        .param("keyword", "노트북")
                        .param("page", "0")
                        .param("size", "5"))
    ...
    @Test
    @WithMockUser(value = "user2@example.com")
    @DisplayName("받은 예약 상태별 필터링")
    void getReceivedReservationsWithStatusTest() throws Exception {
        Long postId = 1L; // 멤버1의 포스트, 멤버2가 예약함 (reservation ID: 4)
    ...
    void updateReservationTest() throws Exception {
        Long reservationId = 2L; // 멤버1의 예약
        LocalDateTime newStartAt = LocalDateTime.now().plusDays(2);
        LocalDateTime newEndAt = LocalDateTime.now().plusDays(9);
